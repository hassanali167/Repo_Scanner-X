...
def verify_github_repo(repo_url, oauth_token=None):
    """Checks if the GitHub repository is accessible (public/private)."""
    ...
    if response.status_code == 200:
        ...
        return "🔒 Private repository detected. Provide a valid OAuth token."
    else:
        return f"⚠️ Error: {response.json().get('message', 'Unknown error occurred.')}"
...
def process_input(project_name, repo_url, oauth_token, commands, uploaded_file, scanners):
    ...
    if "❌" in repo_status or "🔒" in repo_status:
