...
def execute_commands(commands):
    """Executes a list of commands sequentially in the terminal and returns the output."""
    result = ""
    for command in commands.split("\n"):
        ...
        if command:
            try:
                ...
                result += process.stdout if process.stdout else ""
                result += process.stderr if process.stderr else ""
            except Exception as e:
                result += f"\n❌ Error executing `{command}`: {str(e)}\n"
    return result

def process_input(project_name, repo_url, oauth_token, commands, uploaded_file, scanners):
    """Handles user input, verifies GitHub repo, and executes commands sequentially."""
